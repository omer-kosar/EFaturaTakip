...
namespace EFaturaTakip.API.Validations.Stock
{
    public class StockAddValidator : AbstractValidator<DTO.Stock.StockAddDto>
    {
        public StockAddValidator()
        {
            ...
            RuleFor(c => c.Price).NotNull().WithMessage("Fiyat boş olamaz").NotEmpty().WithMessage("Fiyat alanı boş olamaz."); //todo  check whether it will be seen when price is empty
            RuleFor(c => c.Unit).NotNull().WithMessage("Birim alanı boş olamaz.");
            RuleFor(c => c.ValueAddedTax).NotNull().WithMessage("Kdv alanı boş olamaz.").NotEmpty().WithMessage("Kdv alanı boş olamaz.");
        }
    }
}
